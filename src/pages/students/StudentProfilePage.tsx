...
export default function StudentProfilePage() {
  ...
  const { data: attendance, isPending: attendancePending, isError: attendanceError, refetch: refetchAttendance } = useQuery({
    queryKey: queryKeys.students.attendance(studentId),
    queryFn: () => schoolApi.attendance.history(studentId),
    enabled: Number.isInteger(studentId) && studentId > 0
  });
  const attendanceLoading = isInitialLoad(attendancePending, attendance);
  const attendanceFailed = isFailedLoad(attendanceError, attendance);
